...
function loadTiles(n) {
    ...
    selectedTileId = 'square' + highlighted;
...
function checkHasWon() {
    for (let b = 1; b <= numberOfTiles; b++) {
        currentTile = document.getElementById(`square${b}`);
...
function setSelected(index) {
    currentTile = document.getElementById(`square${highlighted}`);
    currentTileText = currentTile.innerHTML;
    currentTile.classList.remove('selected');
    
    newTile = document.getElementById(`square${index}`);
    ...
    // Get the computed background styles
    var background1 = window.getComputedStyle(newTile).background;
    var background2 = window.getComputedStyle(currentTile).background;

    // Swap backgrounds
    newTile.style.background = background2;
    currentTile.style.background = background1;
